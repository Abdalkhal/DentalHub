...
function openFavorite(f: FavItem) {
  switch (f.kind) {
    case 'brand':
      router.push({ pathname: '/brand/[brandId]', params: { brandId: f.id } });
      return;
    case 'product':
    case 'implant':
      router.push({ pathname: '/product-detail/[productId]', params: { productId: f.id } });
      return;
    case 'office':
      router.push({ pathname: '/profile/[accountId]', params: { accountId: f.id } });
      return;
    default:
      // Older entries saved before `kind` existed were brands only.
      router.push({ pathname: '/brand/[brandId]', params: { brandId: f.id } });
  }
}
  ...
  return (
    <Screen>
      <View className="space-y-3">
        {favs.map((f) => (
          <Pressable key={f.id} onPress={() => openFavorite(f)}>
