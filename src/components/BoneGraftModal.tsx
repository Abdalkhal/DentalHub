...
export function BoneGraftModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-[#17324A]/40 backdrop-blur-sm md:items-center md:p-6" onClick={onClose}>
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-[440px] max-h-[92vh] flex flex-col rounded-t-3xl overflow-hidden animate-in slide-in-from-bottom md:max-w-2xl md:max-h-[88vh] md:rounded-3xl md:shadow-2xl lg:max-w-3xl"
        style={{ background: "linear-gradient(to bottom, #F7FCFF, #DCEEFB, #BFE1F7)" }}
      >
        <div
          className="flex items-center justify-between px-4 pt-4 pb-3 shrink-0 text-white md:px-6 md:pt-5 md:pb-4"
          style={{ background: "linear-gradient(to right, #2E93E0, #1C6FB5)" }}
        >
          <h2 className="font-display font-extrabold text-base lg:text-xl">
            {isEdit
              ? ar
                ? "تعديل بون كرافت"
                : "Edit Bone Graft"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 md:grid md:grid-cols-2 md:gap-4 md:space-y-0 md:px-6 md:py-6 md:items-start">
          {/* Section 1 */}
          ...
          {/* Section 5 */}
          <section className={cn(sectionCls, "md:col-span-2")}>
        ...
        <div className="p-4 border-t border-[#D3E8F7] bg-white shrink-0 md:px-6 md:py-4">
          <button
            onClick={submit}
            disabled={busy}
            className="w-full h-12 rounded-2xl text-white font-display font-extrabold text-sm shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-60 transition hover:opacity-95 md:max-w-xs md:mx-auto md:h-11"
