...
import { ArrowLeft, ArrowRight, Globe, LogIn, Search } from "lucide-react";
...
export function TopBar({ title, showSearch = false, showBack = false, variant = "page", searchValue, onSearchChange, searchPlaceholder }: Props) {
  ...
  return (
    <header
      className={cn(
        ...
        variant === "home" ? "bg-gradient-to-b from-primary-soft/70 to-transparent" : "bg-background/90 backdrop-blur border-b border-border"
      )}
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/login"
            className="flex items-center gap-1.5 h-9 px-3 rounded-full bg-primary text-primary-foreground text-xs font-semibold hover:opacity-90"
          >
