...
function StaffPage() {
  ...
  const { members, loading, refetch } = useLabMembers(labId);
  ...
  return (
    <MobileShell wide>
      <div className="min-h-svh bg-slate-50 overflow-x-hidden" dir={dir}>
        ...
        <div className="max-w-6xl mx-auto p-4 lg:p-8 space-y-6 md:px-6 md:pt-6">
          {/* Stats bar */}
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              placeholder={
                ar ? "ابحث باسم العضو أو الوظيفة..." : "Search by member name or role..."
              }
...
function MemberFormModal({
  ...
  const handleSubmit = async () => {
    ...
    if (!editing) {
      ...
      if (password.length < 6) {
        toast.error(
          ar ? "الرمز يجب أن يكون 6 أحرف على الأقل" : "Password must be at least 6 characters",
        );
        return;
      }
      if (password !== confirmPassword) {
        toast.error(ar ? "الرمز وتأكيده غير متطابقين" : "Password and confirmation don't match");
        return;
      }
    }
    setBusy(true);
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-md md:max-w-2xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[92svh] flex flex-col animate-in slide-in-from-bottom duration-300"
      ...
      >
            ...
            <>
              <div>
                <label className="text-xs font-bold text-slate-600 mb-1.5 flex items-center gap-1.5">
                  ...
                  {ar ? "الرمز (٦ أحرف على الأقل)" : "Password (min. 6 characters)"}
                </label>
                <input
                  type="password"
                  value={password}
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 mb-1.5 flex items-center gap-1.5">
                  <ShieldCheck className="size-3.5" />
                  {ar ? "تأكيد الرمز" : "Confirm Password"}
                </label>
                <input
                  type="password"
        ...
        <div className="flex items-center gap-2 px-5 py-4 border-t border-slate-100 shrink-0">
          ...
          >
            ...
            {editing ? (ar ? "حفظ التعديلات" : "Save Changes") : ar ? "تأكيد" : "Confirm"}
          </button>
        </div>
      </div>
    </div>
