...
function DentistOrders() {
  ...
  return (
    <MobileShell>
      <TopBar title={ar ? "تتبع حالاتك" : "Track your cases"} />
      <div className="px-3 pt-4 pb-6">
        <div className="grid grid-cols-3 gap-2 mb-4">
          {statuses.slice(1).map((s) => (
            <button key={s.id} onClick={() => setFilter(s.id)} className={cn("rounded-2xl p-3 text-center transition border-2", filter === s.id ? "bg-primary text-primary-foreground border-primary" : "bg-white text-slate-600 border-slate-200")}>
              <p className="font-extrabold text-xl">{counts[s.id]}</p>
        ...
        {filtered.length === 0 ? (
          <div className="py-12 text-center text-slate-400"><p className="font-semibold">{ar ? "لا توجد حالات" : "No cases"}</p></div>
        ) : (
          <div className="space-y-2">
            {filtered.map((o) => (
              <div key={o.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">{o.doctor} · {o.workType}</p>
                    {o.clinic && <p className="text-[10px] text-slate-400">{ar ? "عيادة" : "Clinic"}: {o.clinic}</p>}
                  </div>
                  <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full", o.status === "completed" ? "bg-emerald-100 text-emerald-700" : o.status === "in_progress" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700")}>
                    {ar ? (o.status === "completed" ? "مكتملة" : o.status === "in_progress" ? "قيد التنفيذ" : "متأخرة") : (o.status === "completed" ? "Completed" : o.status === "in_progress" ? "In Progress" : "Delayed")}
                  </span>
                </div>
