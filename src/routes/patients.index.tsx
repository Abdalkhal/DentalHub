  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-foreground/40 backdrop-blur-sm" onClick={onClose}>
      <form
        ...
        className="w-full max-w-[440px] max-h-[80vh] flex flex-col rounded-t-3xl bg-background"
      >
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <Field label={ar ? "العمر" : "Age"}>
              <input type="number" min={0} value={form.age} onChange={(e) => setForm({ ...form, age: e.target.value === "" ? "" : Number(e.target.value) })} className={inputCls} />
            </Field>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {HISTORY_LABELS.map((h) => {
                ...
                return (
                  <button type="button" key={h.key} onClick={() => setForm({ ...form, history: { ...form.history, [h.key]: !on } })}
                    className={cn("h-10 px-3 rounded-xl border text-[12px] font-semibold text-start transition", on ? "bg-primary/10 border-primary text-primary" : "bg-card border-border text-foreground")}>
                    {ar ? h.ar : h.en}
                  </button>
                );
              })}
          ...
          <Field label={ar ? "الشكوى الرئيسية / ملاحظات" : "Chief complaint / notes"}>
            <textarea rows={3} value={form.complaint} onChange={(e) => setForm({ ...form, complaint: e.target.value })} className={cn(inputCls, "h-auto py-2 resize-none")} />
          </Field>
        </div>

        <div className="p-4 border-t border-border bg-background shrink-0 pb-8 mb-20">
